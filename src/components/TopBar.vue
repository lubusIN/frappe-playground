<template>
  <div
    class="grid min-w-0 grid-cols-[max-content_minmax(0,1fr)] items-center gap-3.5 border-b border-gray-900 dark:border-gray-950 bg-gray-800 dark:bg-gray-900 px-2.5 py-1.5 max-sm:grid-cols-1 max-sm:content-center max-sm:gap-1.5"
  >
    ...
    >
      ...
      <input
        ...
        class="w-full rounded-md border border-gray-600 dark:border-gray-700 bg-gray-700 dark:bg-gray-800 px-2.5 py-0.5 font-mono text-[13px] text-gray-200 placeholder-gray-400 focus:border-gray-400 focus:outline-none focus:ring-0 disabled:opacity-50 transition-colors"
        @input="$emit('update:address', $event.target.value)"
      />
      <Button
        class="w-8 !text-gray-300 hover:!text-white hover:!bg-gray-700 dark:hover:!bg-gray-800"
        type="button"
        variant="ghost"
        :disabled="!ready"
        title="Reload frame"
...
import { RotateCw, Sun, Moon } from '@lucide/vue'
...
defineEmits(['navigate', 'reload', 'update:address', 'toggleTheme'])
</script>